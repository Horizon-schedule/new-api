...
export function processChartData(
  ...
  const modelColorSpecified = buildModelColorMap([...sortedModels, otherLabel])
  if (!modelColorSpecified[otherLabel]) {
    modelColorSpecified[otherLabel] = modelToColor(otherLabel)
  }
  const otherTooltipColor =
    modelColorSpecified[otherLabel] ?? MODEL_CHART_BASE_COLORS[2]
  const modelColor = {
    specified: modelColorSpecified,
  }
