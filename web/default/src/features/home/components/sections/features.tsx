  ...
  const features = [
    {
      ...
      visual: (
        <div className='mt-4 grid grid-cols-3 gap-2'>
          {FEATURE_PROVIDER_NAMES.slice(0, 6).map((name) => (
            <div
              key={name}
              className='home-theme-pill model-orbit-badge flex h-7 w-full items-center justify-center gap-1.5 px-2'
              style={
                {
                  ['--pill-accent' as string]: modelToColor(name.toLowerCase()),
                } as CSSProperties
              }
            >
              <span
                className='home-theme-pill-dot'
                style={{ backgroundColor: modelToColor(name.toLowerCase()) }}
                aria-hidden='true'
              />
              <span className='home-theme-pill-label truncate text-[11px]'>
                {name}
              </span>
            </div>
          ))}
